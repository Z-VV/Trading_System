...
def MainWhile():
    ...
    while True:
        ...
        if currenttime.second==10 :
            if (currenttime.hour+2) % 4==0 and currenttime.minute==0:
                print(currenttime)
                for x in symbols:
                    x.killH4()
                    x.dataH4()
            ...
            for x in symbols:
                ...
                if x.Orders==True:
                    list_of_orders,orders_symbol_list=orders_list()
                    x.Orders=delete_orders(x.symbol,list_of_orders)
                if (x.ready30b and x.ready15b and x.ready5b and x.ready1b) or (x.ready30s and x.ready15s and x.ready5s and x.ready1s) and x.stop:
                    symbols_in_DB,tuple_of_orders_in_DB=Database_Info_algo_m1()
